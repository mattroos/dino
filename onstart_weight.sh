...
aws s3 cp s3://bicog-datasets/cows/video_info_2021-Feb-Mar.csv ./


# Delete the onstart.sh file to remove AWS credentials?
# rm /root/onstart_weight.sh
...
# Download DINO code
# Need to do this manually, so can respond to "Are you sure you want to continue connecting (yes/no)?""
cd /workspace
